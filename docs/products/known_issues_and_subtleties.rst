...
Right Ascension errors in DP1 suffer from an over-application of the cosine of the declination (cosδ) geometric correction. Consequently, the reported uncertainties (e.g., ``coord_raErr`` in :ref:`catalogs-object` catalog and ``raErr`` in :ref:`catalogs-dia-source` catalog) include an extra scaling factor. To recover the correct on-sky uncertainty, users must divide the reported RA error by cosδ. This issue will be resolved in DP2 and future data releases.
