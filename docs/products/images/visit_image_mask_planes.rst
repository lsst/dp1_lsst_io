...
``CLIPPED``
    ...
    However, this may be this may be back-propagated on visit and differences images in future data releases.
...
``INTRP``
    ...
    flagged ``BAD``, ``SAT``, or ``CR``).
    After interpolation, the pipeline sets the ``INTRP`` bit to indicate the
...
``NOT_DEBLENDED``
    ...
    If a detected object was too large,
...
``SUSPECT``
    ...
    The ``SUSPECT`` bit is set for pixels above the PTC turnoff
    ...
    This means ``SUSPECT`` pixels are typically on the shoulders or flanks of saturated regions, where flux is high
