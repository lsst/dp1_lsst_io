...
This tutorial explores the LSSTComCam observations for the "Rubin_SV_38_7" field, including magnitude limits, visit distribution with time, data quality, and the distributions of stars and galaxies in color-magnitude and color-color diagrams.

This field is also referred to as the "low ecliptic latitude field" because it was close to the ecliptic plane and used an alternative dither pattern to maximize the coverage of Solar System objects and to facilitate testing Solar System Object linking across multiple nights during ComCam on-sky commissioning.

**Central coordinates:** (RA, Dec) = 37.98, 7.015 degrees
...
`Go to the gri color HiPS map for the "Rubin_SV_38_7" field <https://data.lsst.cloud/portal/app/?api=hips&uri=https://data.lsst.cloud/api/hips/v2/dp1/deep_coadd/color_gri&ra=37.98&dec=7.015&sr=50m>`_.
...
This query will retrieve all images of subtype ``deep_coadd`` that contain the central coordinates of the "Rubin_SV_38_7" field.
...
.. code-block:: SQL
  ...
  WHERE obs_collection = 'LSST.DP1' AND calib_level = 3 AND dataproduct_type = 'image'
        ...
        AND CONTAINS(POINT('ICRS', 37.98, 7.015), s_region)=1
...
.. figure:: images/portal-301-2-1.png
    :name: portal-301-2-1
...
Use the image display interface to zoom, pan, rescale, and generally explore the deep images of the "Rubin_SV_38_7" field.
...
This query will retrieve all images of subtype ``deep_coadd`` that were obtained with the *r*-band filter and overlap the ~1 degree "Rubin_SV_38_7" field (not just its central coordinates).
...
This query will retrieve the coordinates, band, and MJD for all visits from the ``Visit`` table with central coordinates within the "Rubin_SV_38_7" field.
...
This query retrieves a table of all CcdVists (visit and detector combinations) that were observed of the "Rubin_SV_38_7" field.
...
Filter the table for only point-like objects ("stars") by filtering the ``refExtendedness`` column to be equal to 0.
