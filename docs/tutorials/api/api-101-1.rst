...
Note that the column names, data types, units, and descriptions for the columns of the dp1.CcdVisit table are shown in the right-hand panel. Click the tabs to change the view.
...
**8. In the ADQL Text panel of the Table Access Protocol (TAP) Query window, click the Examples button at the bottom left and select Full Table from the Basic menu.**
A sample ADQL query will appear in the text box, returning all column names from the selected table. Because some of the tables contain a large number of columns, this query type is generally not recommended.

You can modify the query directly in the ADQL text box or use a predefined query (such as the one shown in Step 9).
Explore the other options in the Examples menu to try additional basic ADQL queries available through the TAP service.
...
**9. Avoid full-table queries. Construct a specific query to access information from the CCDVisit table.**

Although the DP1 CcdVisit table is relatively small, it is good practice to always include spatial constraints and only retrieve necessary columns.
...
.. code-block:: sql
  ...
  SELECT ccdVisitId, visitId, detector,
