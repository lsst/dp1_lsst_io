...
The results object contains a table of image metadata. Print the number of results and inspect the columns.

.. code-block:: python

   print(f"Found {len(results)} images")
   print(results.to_table().colnames)
...
9. Access individual images
----------------------------

To retrieve a specific image, use the ``access_url`` from the results. This URL points to a datalink service that provides various access methods for the image.

.. code-block:: python

   # Get the access URL for the first image
   image_access_url = results[0].access_url
   print(image_access_url)

The datalink service can be queried to obtain the direct download URL for the image file.

For a complete example of downloading and displaying images using PyVO and the SIA service, see the notebook tutorial :doc:`/tutorials/notebook/103/notebook-103-2`.
...
1. Create and configure an RSP access token for SIA access
2. Set up PyVO with authentication to access the Rubin SIA service
3. Query for images by spatial region and calibration level
4. Inspect the returned image metadata

The SIA service provides a standardized way to discover and access images from the Rubin Science Platform using Virtual Observatory protocols.
