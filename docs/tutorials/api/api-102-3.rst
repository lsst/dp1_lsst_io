...
Two spatial cutout shapes are supported: circle and polygon. The circle shape produces a square cutout with edge size equal to the circle diameter. The polygon shape produces rectangular cutouts defined by vertices ordered in the counter-clockwise direction.
...
Search for a calibrated image at the target position. For this example, search for images in a circular region centered at RA, Dec = 53.076, -28.110 degrees.
...
.. code-block:: python
   ...
   results = sia_service.search(
       pos=(target_ra, target_dec, 0.05), calib_level=2, maxrec=1
   )
...
9. Create a polygon cutout
----------------------------

Polygon cutouts enable rectangular cutout regions. Define the polygon vertices as pairs of RA, Dec coordinates in counter-clockwise order.

Create a new SODA query and define a rectangular polygon cutout.

.. code-block:: python

   sq_poly = SodaQuery.from_resource(
       dl_result,
   ...
   sq_poly.polygon = (
       target_ra * u.deg - ra_edge,
       target_dec * u.deg - de_edge,
       target_ra * u.deg - ra_edge,
       target_dec * u.deg + de_edge,
       target_ra * u.deg + ra_edge,
       target_dec * u.deg + de_edge,
...
Execute the polygon cutout query and display the result.

.. code-block:: python

   cutout_bytes_poly = sq_poly.execute_stream().read()
   sq_poly.raise_if_error()
   hdul_poly = fits.open(io.BytesIO(cutout_bytes_poly))
   image_poly = hdul_poly[1].data
   ...
   plt.imshow(
       ...
       cmap="gray",
       origin="lower",
       vmin=np.percentile(image_poly, 5),
       vmax=np.percentile(image_poly, 95),
   )
   plt.title("Rectangular Polygon Cutout")
   plt.colorbar()
   plt.show()

.. note::

   The circle shape definition accounts for cos(declination) automatically, but the polygon definition does not. To match the angular size of a circular cutout in the RA direction when using a polygon, divide the RA edge length by cos(dec):

   .. code-block:: python

      a = Angle(target_dec, u.deg)
      cosd = np.cos(a.radian)
      ra_edge_corrected = ra_edge / cosd

10. Exercises for the learner
------------------------------

**10.1.** Retrieve a cutout centered at RA, Dec = 59.1, -48.8 degrees with a radius of 0.03 degrees. Display the result.

**10.2.** Repeat the cutout from Step 7 using the ``cutout-sync-maskedimage`` service type and display the mask plane (the third HDU extension).
